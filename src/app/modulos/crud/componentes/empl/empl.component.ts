...
export class EmplComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._naeService.registros().subscribe(naes => { this.naes = naes; });
    this._remService.registros().subscribe(rems => { this.rems = rems; });
    this._taeService.registros().subscribe(taes => { this.taes = taes; });
    this._temService.registros().subscribe(tems => { this.tems = tems; });
    this._munService.municipios().subscribe(muns => { this.muns = muns; });
    ...
    */
    ...
    $('#registros').DataTable({
      ...
      'bProcessing': true,
      'bServerSide': true,
      // 'sAjaxSource': JSON.stringify({ empls }),
  ...
  public consultarEmpleador() {
    ...
    this._emplService.registro(TipoID, NumeroIdentificacion).subscribe(empls => {
      empls.forEach(empl => {
        this.empl = empl;
      });
      this.cargando = false;
  ...
  refrescarEmpleadores() {
    this._emplService.registros().subscribe(empls => { this.empls = empls; console.log(this.empls); });
  }
