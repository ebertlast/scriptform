...
import { Tae as Model } from '../modelos/tae';
...
export class TaeService {
  ...
  public nuevoRegistro(model: Model): Observable<boolean> {
    ...
    const _url = environment.apiurl + '/tamanoempresa/nuevo';
  ...
  public registros(TamanoID: string = ''): Observable<Model[]> {
    const _headers = new Headers({ 'Authorization': 'Bearer ' + this._authService.Usuario().TOKEN });
    const _options = new RequestOptions({ headers: _headers });
    const _url = environment.apiurl + '/tamanoempresa/' + TamanoID;
  ...
  public eliminarRegistro(TamanoID: string): Observable<boolean> {
    const _headers = new Headers({
      'Authorization': 'Bearer ' + this._authService.Usuario().TOKEN
    });
    const _options = new RequestOptions({ headers: _headers });
  ...
  public actualizarRegistro(model: Model): Observable<boolean> {
    ...
    const _options = new RequestOptions({ headers: _headers });
    const _json = 'json=' + JSON.stringify({ model });
    const _url = environment.apiurl + '/tamanoempresa/actualizar';
