...
import { Tdi as Model } from '../modelos/tdi';
...
export class TdiService {
  ...
  public nuevoRegistro(model: Model): Observable<boolean> {
    ...
    const _url = environment.apiurl + '/tipodiscapacidad/nuevo';
  ...
  public registros(TipoDiscapacidadId: string = ''): Observable<Model[]> {
    const _headers = new Headers({ 'Authorization': 'Bearer ' + this._authService.Usuario().TOKEN });
    const _options = new RequestOptions({ headers: _headers });
    const _url = environment.apiurl + '/tipodiscapacidad/' + TipoDiscapacidadId;
  ...
  public eliminarRegistro(TipoDiscapacidadId: string): Observable<boolean> {
    const _headers = new Headers({
      'Authorization': 'Bearer ' + this._authService.Usuario().TOKEN
    });
    const _options = new RequestOptions({ headers: _headers });
  ...
  public actualizarRegistro(model: Model): Observable<boolean> {
    ...
    const _options = new RequestOptions({ headers: _headers });
    const _json = 'json=' + JSON.stringify({ model });
    const _url = environment.apiurl + '/tipodiscapacidad/actualizar';
