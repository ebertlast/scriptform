...
import { Usproh as Model } from '../modelos/usproh';
...
export class UsprohService {
  ...
  public nuevoRegistro(model: Model): Observable<boolean> {
    ...
    const _url = environment.apiurl + '/detalleprocedimientos/nuevo';
  ...
  public registros(ProcedimientoID: string, ControlID: string = ''): Observable<Model[]> {
  ...
  public eliminarRegistro(ProcedimientoID: string, ControlID: string): Observable<boolean> {
    const _headers = new Headers({
      'Authorization': 'Bearer ' + this._authService.Usuario().TOKEN
    });
    const _options = new RequestOptions({ headers: _headers });
  ...
  public actualizarRegistro(model: Model): Observable<boolean> {
    ...
    const _options = new RequestOptions({ headers: _headers });
    const _json = 'json=' + JSON.stringify({ model });
    const _url = environment.apiurl + '/detalleprocedimientos/actualizar';
